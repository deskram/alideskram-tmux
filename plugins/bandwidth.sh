...
if [[ $(uname -s) == "Darwin" ]]; then
    network_name=$(get_tmux_option "@alideskram-bandwidth-network-name" "en1")
    ...
    network_name=$(get_tmux_option "@alideskram-bandwidth-network-name" "${default_network_device}")
else
...
main() {
    RATE=$(get_tmux_option "@alideskram-refresh-rate" 1) # seconds
    while true; do
        output_download=""
        output_upload=""
        ...
        up_icon=$(get_tmux_option "@alideskram-bandwidth-up-icon" "")
        down_icon=$(get_tmux_option "@alideskram-bandwidth-up-icon" "")
